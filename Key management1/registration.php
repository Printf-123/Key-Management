...
if (isset($_POST["submit"])) {
    ...
    if (count($errors) > 0) {
        ...
        $sql = "INSERT INTO users (name, idnum, section, email) VALUES (?, ?, ?, ?)";
        $stmt = mysqli_stmt_init($conn_login_register);
        if (mysqli_stmt_prepare($stmt, $sql)) {
            mysqli_stmt_bind_param($stmt, "ssss", $fullname, $idnum, $section, $email);
...
?>
        ...
        <input type="email"placeholder="Email" name="email" required></h2>
        <button type="submit" name="submit">Register</button>
      </form>
        <div><p>Already Registered <a href="borrow.php">Borrow Here</a></p></div>
</div>
</body>
</html>