...
if (isset($_POST["return"])) {
    ...
    if (empty($borrower_id) || empty($section) || empty($key_name)) {
        echo "<script>alert('Please fill in all fields!');</script>";
    } else {
        // Update the return_date instead of deleting the record
        $return_date = date('Y-m-d H:i:s');
        $sql = "UPDATE users SET return_date = ? WHERE key_name = ? AND borrower_id = ? AND return_date IS NULL";
        $stmt = mysqli_stmt_init($conn_key_records);
        if (mysqli_stmt_prepare($stmt, $sql)) {
            mysqli_stmt_bind_param($stmt, "sss", $return_date, $key_name, $borrower_id);
